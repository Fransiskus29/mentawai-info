...
# ================= MENU 1: DASHBOARD (DENGAN GRAFIK!) =================
if menu == "🏠 Dashboard":
    ...
    # === FITUR BARU: GRAFIK TREN HARGA ===
    st.subheader("📈 Grafik Tren Harga (Real-Time)")
    st.caption("Lihat pergerakan harga berdasarkan laporan warga.")
    
    # Ambil Data
    if db:
        docs = db.collection('mentawai_v2').order_by('waktu').stream() # Urutkan dari lama ke baru buat grafik
        data_all = []
        for d in docs:
            dt = d.to_dict()
        ...
        if not df.empty:
            # Filter Grafik
            pilihan_grafik = st.selectbox("Pilih Komoditas untuk Grafik:", df['Barang'].unique())
            ...
            if not df_chart.empty:
                ...
                # Tampilkan Detail Tabel di Bawahnya
                with st.expander(f"Lihat Detail Data {pilihan_grafik}"):
                    # Format ulang tabel biar cantik
            ...
            st.warning("Belum ada data untuk ditampilkan di grafik. Silakan input laporan dulu.")
...
# ================= MENU 2: KALKULATOR =================
    ...
    with c_kiri:
        ...
        if harga_pusat == 0:
            st.warning("⚠️ Harga acuan belum diset Admin.")
        else:
