...
if menu == "🏠 Dashboard":
    ...
    with tab1:
        ...
        with c5: st.markdown(f"""<div class="acuan-box"><div class="label-kecil">KAKAO</div><div class="harga-besar">Rp {acuan_data.get('Kakao (Coklat)', 0):,}</div></div>""", unsafe_allow_html=True)
        with c6: st.markdown(f"""<div class="acuan-box"><div class="label-kecil">GAMBIR</div><div class="harga-besar">Rp {acuan_data.get('Gambir', 0):,}</div></div>""", unsafe_allow_html=True)
        with c7: st.markdown(f"""<div class="acuan-box"><div class="label-kecil">NILAM</div><div class="harga-besar">Rp {acuan_data.get('Nilam', 0):,}</div></div>""", unsafe_allow_html=True)
        with c8: st.markdown(f"""<div class="acuan-box"><div class="label-kecil">SAGU</div><div class="harga-besar">Rp {acuan_data.get('Sagu', 0):,}</div></div>""", unsafe_allow_html=True)
    ...
    # FILTER BARU
    filter_jenis = st.multiselect("Cari Barang:", LIST_KOMODITAS)
    ...
    if db:
        ...
        for d in docs:
            ...
            data_table.append({
                ...
                "Sumber/Catatan": dt.get('catatan', '-'),
                "Waktu": dt.get('waktu').strftime("%d/%m") if dt.get('waktu') else "-"
            })
...
# ================= MENU 2: KALKULATOR =================
    ...
    with c_kiri:
        ...
        # Ambil harga langsung pakai nama komoditas sebagai KEY
        harga_pusat = acuan_data.get(kom, 0)
        
        if harga_pusat == 0:
            st.warning("⚠️ Harga acuan belum diset Admin. Hubungi Admin!")
        else:
...
# ================= MENU 5: UPDATE HARGA (KOMPLIT) =================
    ...
    with st.form("update_komplit"):
        ...
        st.divider()
        st.subheader("🦅 Hasil Hutan ('Udara')")
        ...
        if st.form_submit_button("SIMPAN SEMUA HARGA 💾"):
            data_baru = {
            ...
            st.success("Database Lengkap Terupdate!")
            st.rerun()
